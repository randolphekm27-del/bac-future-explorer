          ...
          <div className="flex flex-wrap gap-2">
            {university.schools?.[0]?.contact?.phone && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 px-3 text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 flex items-center justify-center gap-2"
                onClick={(e) => handleContactClick(e, university.schools[0].contact.phone, 'phone')}
                data-testid="button-phone"
              >
                <Phone className="h-3 w-3 flex-shrink-0 mt-0" />
                <span className="font-bold leading-3 flex items-center">Appeler</span>
              </Button>
            )}
            ...
            {university.schools?.[0]?.contact?.email && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 px-3 text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 flex items-center justify-center gap-2"
                onClick={(e) => handleContactClick(e, university.schools[0].contact.email, 'email')}
                data-testid="button-email"
              >
                <Mail className="h-3 w-3 flex-shrink-0 mt-0" />
                <span className="font-bold leading-3 flex items-center">Email</span>
              </Button>
            )}
            ...
            {university.schools?.[0]?.contact?.website && (
              <Button
                variant="ghost"
                size="sm"
                className="h-8 px-3 text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 flex items-center justify-center gap-2"
                onClick={(e) => handleContactClick(e, university.schools[0].contact.website, 'website')}
                data-testid="button-website"
              >
                <Globe className="h-3 w-3 flex-shrink-0 mt-0" />
                <span className="font-bold leading-3 flex items-center">Site web</span>
              </Button>
            )}
          </div>
        </div>
